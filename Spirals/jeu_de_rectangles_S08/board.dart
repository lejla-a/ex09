    ...
    Board() 
    {
        ...
        new Timer.periodic(const Duration(milliseconds: 1000), (t) 
        {
            if (!stopped && rectangleRouge.big) 
            {
                ...
                seconds++;
                score.update(collisionCount, minutes, seconds, rectangles.length, niveau);
                timeLabel.text = '${score.seconds}';
                
                if (rectangleRouge.validerSortie(zone_sortie)) 
                {
                    niveau++;
                    niveauLabel.text = niveau.toString();
                    score.zero();
                    start_restart();
                    msgLabel.text = 'Vous avez gagné en ' + timeLabel.text + ' secondes!';
                    
                    if (score.betterTimeThan(bestScore)) 
                    {
                        bestScore.update(collisionCount, minutes, seconds, rectangles.length, niveau);
                        bestScore.save();
                        bestScoreSection.display();
                    } 
